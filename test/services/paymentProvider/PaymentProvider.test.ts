  ...
  describe("test stripeWebhook", () => {
    ...
    test("test api call", () => {
      const scope = nock("https://api-dev.liblab.com")
        ...
        .reply(200, {data: {}});
      return sdk.paymentProvider.stripeWebhook('soluta').then((r: any) => expect(r.data).toEqual({}))
    });
    ...
    test("test will throw error on a non-200 response", () => {
      const scope = nock("https://api-dev.liblab.com")
        .post("/payment-provider/stripe/webhook")
        .reply(404, {data: {}});
      return expect(async () => await sdk.paymentProvider.stripeWebhook('sapiente')).rejects.toThrow();
    });
