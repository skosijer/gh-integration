...
export class BlocklistsService extends BaseService {
    ...
    async getBlocklist(organizationId, blocklistId, optionalParams = {}) {
        ...
        urlEndpoint = urlEndpoint.replace('{organization_id}', serializePath('simple', false, organizationId, undefined));
        urlEndpoint = urlEndpoint.replace('{blocklist_id}', serializePath('simple', false, blocklistId, undefined));
        if (withShared) {
            queryParams.push(serializeQuery('form', true, 'with_shared', withShared));
        }
        if (withPublic) {
    ...
    async unshareBlocklist(organizationId, blocklistId, unshareOrganizationId, optionalParams = {}) {
        ...
        urlEndpoint = urlEndpoint.replace('{organization_id}', serializePath('simple', false, organizationId, undefined));
        urlEndpoint = urlEndpoint.replace('{blocklist_id}', serializePath('simple', false, blocklistId, undefined));
        urlEndpoint = urlEndpoint.replace('{unshare_organization_id}', serializePath('simple', false, unshareOrganizationId, undefined));
        if (force) {
            queryParams.push(serializeQuery('form', true, 'force', force));
        }
        const urlParams = queryParams.length > 0 ? `?${queryParams.join('&')}` : '';
