...
export default class HTTPLibrary implements HTTPClient {
    ...
    readonly retryDelayMs: number = 150;
        ...
        private static readonly responseMapper: Map<string, string> = new Map<string, string>([['type', 'type_'],['response', 'response_']]);

private readonly requestMapper: Map<string, string> = new Map<string, string>([['type_', 'type'],['response_', 'response']]);
  ...
  private static handleResponse(response: any) {
    ...
    private getUserAgentHeader(): Headers {
        const userAgentBase = 'Liblab/9.9.9';
