...
form: (parameterName: string, parameterValue: unknown, explode: boolean) => {
    ...
    if (Array.isArray(parameterValue)) {
        return explode ? parameterValue.map(value => `${parameterName}=${value}`).join('&') : `${parameterName}=${parameterValue.join(',')}`;
    }
    ...
    if (typeof parameterValue === 'object' && parameterValue !== null) {
        if (explode) {
            ...
            return Object.entries(parameterValue).map(([name, value]) => `${name}=${value}`).join('&');
        }
        // Serialize object with non-exploded format: "key=key1,value1,key2,value2"
        return `${parameterName}=${Object.entries(parameterValue).flatMap(([name, value]) => [name, value]).join(',')}`;
    }
...
export function serializeQuery(style: QueryStyles, explode: Explode, key: string, value: unknown): string {
